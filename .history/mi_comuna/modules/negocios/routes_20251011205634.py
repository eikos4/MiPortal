...
def detalle(id):
    ...
    # 🔹 Obtener el perfil asociado al negocio (si existe)
    perfil = getattr(negocio.usuario, "perfil", None) if negocio.usuario else None

    # 🔹 Armamos el feed consolidado (avisos, eventos, noticias, ofertas)
    feed = []

    if perfil:
        # --- Avisos ---
        for a in perfil.avisos.all() if hasattr(perfil.avisos, "all") else perfil.avisos:
            feed.append({
                "tipo": "aviso",
        ...
        # --- Eventos ---
        for e in perfil.eventos.all() if hasattr(perfil.eventos, "all") else perfil.eventos:
            feed.append({
                "tipo": "evento",
        ...
        # --- Noticias ---
        for n in perfil.noticias.all() if hasattr(perfil.noticias, "all") else perfil.noticias:
            feed.append({
                "tipo": "noticia",
                ...
                "imagen": n.imagen,
                "extra": {"fecha": getattr(n, "fecha_publicacion", None)}
            })

        # --- Ofertas ---
        for o in perfil.ofertas.all() if hasattr(perfil.ofertas, "all") else perfil.ofertas:
            vigente_flag = None
            try:
    ...
    feed.sort(key=lambda x: x.get("fecha") or datetime.utcnow(), reverse=True)
