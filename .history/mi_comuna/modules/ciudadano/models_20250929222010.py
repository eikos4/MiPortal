...
class PerfilEmpresa(db.Model):
    ...
    categoria_id = db.Column(db.Integer, db.ForeignKey("categoria.id"), nullable=True)  # 👈 agregado
...
class Evento(db.Model):
    ...
    empresa_id = db.Column(db.Integer, db.ForeignKey("perfil_empresa.id"), nullable=False, index=True)
    empresa = db.relationship("PerfilEmpresa", backref="eventos")

    def __repr__(self):
        return f"<Evento {self.titulo} ({self.empresa_id})>"
...
class Aviso(db.Model):
    ...
    def __repr__(self):
        return f"<Aviso {self.titulo} ({self.empresa_id})>"
...
class NoticiaEmpresa(db.Model):
    ...
    fecha_publicacion = db.Column(db.Date, default=datetime.utcnow, index=True)

    empresa_id = db.Column(db.Integer, db.ForeignKey("perfil_empresa.id"), nullable=False, index=True)
    empresa = db.relationship("PerfilEmpresa", backref="noticias")

    def __repr__(self):
        return f"<NoticiaEmpresa {self.titulo} ({self.empresa_id})>"
...
class Oferta(db.Model):
    ...
    def __repr__(self):
        return f"<Oferta {self.titulo} ({self.empresa_id})>"
