...
class NoticiaForm(FlaskForm):
    ...
    imagen = FileField("Imagen (opcional)")
    fecha_publicacion = DateField("Fecha publicación", format="%Y-%m-%d", validators=[Optional()])
...
class EventoForm(FlaskForm):
    titulo = StringField("Título del evento", validators=[DataRequired(), Length(max=200)])
    descripcion = TextAreaField("Descripción", validators=[DataRequired()])
